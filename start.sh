...
clean_temp() {
    printf "${CYAN}[*] Limpando arquivos temporários antigos para melhor performance...${NC}\n"
    mkdir -p ./util/temp/sys
    find ./util/temp/sys -type f -mmin +60 -delete 2>/dev/null || true
    rm -f ./temp/baileys_store.json ./temp/floodControl.json 2>/dev/null || true
}
